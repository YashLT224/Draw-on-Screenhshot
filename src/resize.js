...
// When the image is loaded, capture its original size
const handleImageLoad = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
        if (imageRef.current && originalImageSize.width && originalImageSize.height) {
          const newWidth = imageRef.current.clientWidth;
          const newHeight = imageRef.current.clientHeight;
          
          // Update stage to match new image dimensions
          setImageWidth(newWidth);
          setImageHeight(newHeight);
          
          const scaleX = newWidth / originalImageSize.width;
          const scaleY = newHeight / originalImageSize.height;
...
//   };
  ...
  return (
    ...
    <div style={{backgroundColor:'brown' , width:'50%', height:'100vh',overflow:'auto',paddingTop:'20px'}}>
      ...
      <img id="img" ref={imageRef} style={{maxWidth:'100%', height:'auto'}} src={fullpage} alt='d' onLoad={handleImageLoad}/>
      <div id="abc" 
       style={{
        position: 'absolute',
      ...
      >
         <Stage
     width={imageWidth}//numeric value
      height={imageHeight}
